...
try:
    ...
    raise RuntimeError(
        "Efficient-SAM2 import failed. Run install_CPU.bat first.\n" + str(exc)
    )
...
REPO_DIR = os.path.join(ROOT_DIR, "efficient-sam2")

DEFAULT_CHECKPOINT = os.path.join(ROOT_DIR, "model", "sam2.1_hiera_large.pt")
DEFAULT_CONFIG = os.path.join(REPO_DIR, "sam2", "configs", "sam2.1", "sam2.1_hiera_l.yaml")
...
class Sam2Engine:
    def __init__(self, checkpoint_path, config_name):
        if not os.path.exists(checkpoint_path):
            raise FileNotFoundError(
                f"Checkpoint not found: {checkpoint_path}. Run install_CPU.bat."
            )
        if not os.path.exists(config_name):
            raise FileNotFoundError(
                f"Config not found: {config_name}. Make sure Efficient-SAM2 repo exists."
            )
        ...
        self.image_predictor = SAM2ImagePredictor(self.image_model)
    ...
    def prewarm_image(self, image_rgb):
        if image_rgb is None:
            return
        with torch.inference_mode(), self._torch_context():
            self.image_predictor.set_image(image_rgb)
        self._last_image_id = id(image_rgb)
        self._last_image_shape = image_rgb.shape

    def predict_image_mask(self, image_rgb, points, labels, box):
        if image_rgb is None:
            return None

        with torch.inference_mode(), self._torch_context():
            image_id = id(image_rgb)
            if self._last_image_id != image_id or self._last_image_shape != image_rgb.shape:
                self.image_predictor.set_image(image_rgb)
            ...
            if points is None or len(points) == 0:
                ...
                points_tensor = np.asarray(points, dtype=np.float32)
                labels_tensor = np.asarray(labels, dtype=np.int32)

            box_tensor = None
            if box is not None:
                box_tensor = np.asarray(box, dtype=np.float32)
    ...
    def init_video(self, video_path):
        with torch.inference_mode(), self._torch_context():
            ...
            if hasattr(self.video_predictor, "init_memory_info"):
                try:
                    self.video_predictor.init_memory_info(False)
                except Exception:
                    pass
            if not hasattr(self.video_predictor, "time_log"):
                self.video_predictor.time_log = {}
            # Ensure optional Efficient-SAM2 flags exist to avoid AttributeError
            if not hasattr(self.video_predictor, "Mem_Frame_Prune"):
                self.video_predictor.Mem_Frame_Prune = False
            if not hasattr(self.video_predictor, "num_frame_to_prune"):
                self.video_predictor.num_frame_to_prune = 0
            return state

    def predict_video_mask(self, state, frame_idx, points, labels, box):
        with torch.inference_mode(), self._torch_context():
            points_tensor = None
            labels_tensor = None
            ...
            _, _, video_masks = self.video_predictor.add_new_points_or_box(
...
class VideoInitWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            state = self.engine.init_video(self.video_path)
            self.finished.emit(state, "")
        except Exception as exc:
            self.finished.emit(None, str(exc))


class ImagePrepWorker(QtCore.QThread):
    finished = QtCore.pyqtSignal(str)

    def __init__(self, engine, image_rgb):
        super().__init__()
        self.engine = engine
    ...
    def run(self):
        try:
            self.engine.prewarm_image(self.image_rgb)
            self.finished.emit("")
        except Exception as exc:
...
class VideoPropagateWorker(QtCore.QThread):
    ...
    def __init__(self, predictor, state, version_id):
        super().__init__()
    ...
    def run(self):
        try:
            for frame_idx, _, masks in self.predictor.propagate_in_video(self.state):
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.engine = Sam2Engine(DEFAULT_CHECKPOINT, DEFAULT_CONFIG)
        self.is_gpu = self.engine.device == "cuda"
        ...
        self.threshold_timer = QtCore.QTimer(self)
        self.threshold_timer.setSingleShot(True)
        self.threshold_timer.timeout.connect(self.on_threshold_timer)
    ...
    def set_ui_enabled(self, enabled):
        ...
        self.auto_propagate_checkbox.setEnabled(enabled and self.is_video)
        self.apply_video_button.setEnabled(enabled and self.is_video)
    ...
    def _build_ui(self):
        ...
        self.auto_propagate_checkbox = QtWidgets.QCheckBox("動画全体に自動適用")
        self.auto_propagate_checkbox.setChecked(self.is_gpu)
        self.apply_video_button = QtWidgets.QPushButton("動画全体に適用")
        self.apply_video_button.clicked.connect(self.start_propagation)
        ...
        control_layout.addSpacing(8)
        control_layout.addWidget(self.auto_propagate_checkbox)
        control_layout.addWidget(self.apply_video_button)
        control_layout.addSpacing(8)
    ...
    def load_image(self, path):
        ...
        self.auto_propagate_checkbox.setEnabled(False)
        self.apply_video_button.setEnabled(False)
        self.start_image_prep()
    ...
    def load_video(self, path):
        ...
        self.status_label.setText("動画を読み込みました。初期化中...")
        self.auto_propagate_checkbox.setEnabled(True)
        self.apply_video_button.setEnabled(True)
        self.start_image_prep()
        self.start_video_init()

    def on_video_init_finished(self, state, error):
        ...
        if error or state is None:
            self.status_label.setText("動画の初期化に失敗しました。")
            if error:
        ...
        self.push_history()
        self.status_label.setText("動画の準備完了。プロンプトを追加してください。")
        self.set_ui_enabled(True)
        if self.pending_mask_update:
            self.pending_mask_update = False
            self.update_mask(force_predict=True)

    def start_video_init(self):
        if self.video_worker is not None:
            return
        if not self.video_path:
            self.status_label.setText("動画の初期化に失敗しました。")
            return
        self.status_label.setText("動画を初期化中...")
        self.set_ui_enabled(False)
        self.loading_dialog = QtWidgets.QProgressDialog("動画を初期化中...", None, 0, 0, self)
        ...
        self.video_worker = VideoInitWorker(self.engine, self.video_path)
        self.video_worker.finished.connect(self.on_video_init_finished)
        self.video_worker.start()

    def start_image_prep(self):
        if self.prep_worker is not None:
            return
        if self.current_image is None:
            return
        self.status_label.setText("プレビュー準備中...")
        self.set_ui_enabled(False)
        self.prep_dialog = QtWidgets.QProgressDialog("プレビュー準備中...", None, 0, 0, self)
        self.prep_dialog.setWindowTitle("準備中")
        self.prep_dialog.setCancelButton(None)
        self.prep_dialog.setWindowModality(QtCore.Qt.WindowModality.ApplicationModal)
        self.prep_dialog.setMinimumDuration(0)
        self.prep_dialog.show()
        QtWidgets.QApplication.processEvents()
        self.prep_worker = ImagePrepWorker(self.engine, self.current_image)
        self.prep_worker.finished.connect(self.on_image_prep_finished)
        self.prep_worker.start()

    def on_image_prep_finished(self, error):
        if self.prep_dialog is not None:
            self.prep_dialog.close()
            self.prep_dialog = None
        self.prep_worker = None
        self.set_ui_enabled(True)
        if error:
            self.status_label.setText("プレビュー準備に失敗しました。")
        else:
    ...
    def start_propagation(self):
        ...
        self.set_ui_enabled(False)
        self.propagate_worker = VideoPropagateWorker(self.engine.video_predictor, self.video_state, current_version)
    ...
    def on_threshold_changed(self, value):
        ...
        if self.current_image is not None:
            self.threshold_timer.start(180)

    def on_threshold_timer(self):
        if self.current_image is not None:
            self.update_mask(force_predict=False)

    def _mask_to_prob(self, mask_raw):
        if mask_raw is None:
            return None
        m = mask_raw
        ...
        if self.mask_blur_sigma > 0:
            m = cv2.GaussianBlur(m, (0, 0), self.mask_blur_sigma)
        return m
    ...
    def update_mask(self, force_predict=False):
        if self.current_image is None:
            return
        if len(self.points) == 0 and self.box is None:
            self.canvas.set_overlay(None)
            self.current_mask = None
            self.current_mask_raw = None
            self.last_mask_key = None
        ...
        mask_key = (
            ...
            tuple(self.labels),
            self.box,
            self.current_frame_index if self.is_video else None,
            self.is_video,
        )

        threshold = self.threshold_slider.value() / 100
        if not force_predict and self.current_mask_raw is not None and self.last_mask_key == mask_key:
            prob = self._mask_to_prob(self.current_mask_raw)
            mask = None
            if prob is not None:
                mask = prob >= threshold
        ...
        self.status_label.setText("SAM2 実行中...")
        QtWidgets.QApplication.processEvents()
        ...
        if self.is_video:
            if self.video_state is None:
                ...
                self.start_video_init()
                return
            mask_raw = self.engine.predict_video_mask(
                self.video_state,
            ...
            mask_raw = self.engine.predict_image_mask(
                self.current_image,
                self.points,
                self.labels,
                self.box,
        ...
        self.current_mask_raw = mask_raw
        self.last_mask_key = mask_key
        mask = None
        if mask_raw is not None:
            prob = self._mask_to_prob(mask_raw)
            if prob is not None:
                mask = prob >= threshold
        self.current_mask = mask
        if mask is not None:
        ...
        if self.is_video and mask is not None:
            ...
            if self.auto_propagate_checkbox.isChecked():
                self.start_propagation()

    def export_output(self):
        if self.current_image is None:
            self.status_label.setText("画像または動画を読み込んでください。")
            return
    ...
    def export_video(self, mode):
        if self.video_state is None:
            self.pending_mask_update = False
            self.start_video_init()
            self.status_label.setText("動画の初期化中です。完了後に再度出力してください。")
            return
        ...
        threshold = self.threshold_slider.value() / 100
        for frame_idx, _, masks in self.engine.video_predictor.propagate_in_video(
            self.video_state
        ):
            mask = masks[0].cpu().numpy()
            if mask.ndim > 2:
